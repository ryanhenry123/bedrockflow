...
from bedrockflow.security.trivy import (
    TRIVY_IMAGE,
...
def test_run_trivy_default_scan():
    with patch("bedrockflow.security.trivy._trivy_bin", return_value="/usr/bin/trivy"):
        with patch("bedrockflow.security.trivy.subprocess.run") as run:
...
def test_run_trivy_docker_uses_pinned_image():
    with (
        patch("bedrockflow.security.trivy._trivy_bin", return_value=None),
        patch(
            "bedrockflow.security.trivy.shutil.which", return_value="/usr/bin/docker"
        ),
        patch("bedrockflow.security.trivy.subprocess.run") as run,
    ):
        run.return_value.returncode = 0
        run_trivy(use_docker=True, path="/home/ryanh/bedrockflow")
    cmd = run.call_args.args[0]
    assert TRIVY_IMAGE in cmd
    assert cmd[0] == "docker"
    joined = " ".join(cmd)
    assert "/home/ryanh/bedrockflow/.trivyignore" not in joined


def test_dockerize_args_rewrites_paths(tmp_path):
    from bedrockflow.security.trivy import _dockerize_args
...
def test_cli_trivy_subcommand(capsys):
    with patch("bedrockflow.security.trivy.run_trivy", return_value=0) as run:
