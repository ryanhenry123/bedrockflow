# bedrockflow cookbook

Copy-paste recipes for common Bedrock output gates.

- Base install (`pip install bedrockflow`): offline panels and `bedrockflow eval` — no AWS credentials.
- Live Bedrock loops: `pip install "bedrockflow[aws]"` plus configured credentials.
- Retry progress bars: opt in with `BEDROCKFLOW_VISIBLE_TURNS=1` (and `BEDROCKFLOW_PRINT_LAST_DRAFT=1` for stderr dumps).

## 1. JSON extraction

```python
from pydantic import BaseModel
from bedrockflow import Context, converse_with_evals, json_object
...
```bash
bedrockflow eval drafts/answer.json --panel myapp.evals:JSON_PANEL
```
...
```python
from bedrockflow import no_preamble, markdown_sections
...
```python
from bedrockflow import csv_table, markdown_sections
...
```bash
bedrockflow run --example trade_memo \
  --record drafts/latest.md \
  --trace runs/latest.json

bedrockflow eval drafts/latest.md --verbose
...
```yaml
- run: |
    bedrockflow eval tests/fixtures/trade_memo/ \
...
CLI: `bedrockflow run --cache-initial`
...
```python
from bedrockflow import compare_models
...
```python
from bedrockflow import Context, EvalVerdict, gate
...
Filter offline: `bedrockflow eval draft.md --only must_mention_vix`
