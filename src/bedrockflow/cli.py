...
EXAMPLES = {
    "trade_memo": "bedrockflow.examples.trade_memo:main",
    "simple": "bedrockflow.examples.simple_summary:main",
}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    eval_p.add_argument(
        "--panel",
        default="bedrockflow.examples.simple_evals:SIMPLE_EVALS",
    ...
    bandit_p.add_argument(
        "--path",
        default="src/bedrockflow",
        help="Scan path for default recursive scan (default: src/bedrockflow)",
    )
    bandit_p.add_argument(
        "--config",
        type=Path,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "eval":
        from bedrockflow.evals.offline import (
            parse_ctx_json,
            reports_to_json,
            run_eval_cli,
        )
        ...
        if args.report:
            ...
            from bedrockflow.evals.offline import eval_paths, load_panel
    ...
    if args.command == "compare":
        from bedrockflow.examples.compare_runner import (
            compare_example,
            format_compare_rows,
        )
    ...
    if args.command == "trivy":
        from bedrockflow.security.trivy import TrivyNotFoundError, run_trivy
    ...
    if args.command == "bandit":
        from bedrockflow.security.bandit import BanditNotFoundError, run_bandit
